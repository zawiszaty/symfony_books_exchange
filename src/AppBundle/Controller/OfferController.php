...
final class OfferController extends FOSRestController
{
    ...
    public function acceptedOffer(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $offer =  $this->getDoctrine()->getRepository(Offer::class)->find($data['idoffer']);

            $deleteCommand = new DeleteBookCommand($offer->getOfferedBook()->getIdbook());
            $this->get('command_bus')->handle(
                $deleteCommand
            );

            $deleteCommand = new DeleteBookCommand($offer->getRequiredBook()->getIdbook());
            $this->get('command_bus')->handle(
                $deleteCommand
            );
